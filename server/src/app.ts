...
export function createApp() {
  ...
  app.post("/api/analyze", analyzeRateLimit, upload.array("images", 20), async (req, res) => {
    ...
    const vcgBand =
      ...
      (VCG_BAND_OPTIONS as readonly string[]).includes(req.body.vcg_band)
        ...
        : VCG_BAND;
    const customPrompt =
      typeof req.body.custom_prompt === "string" && req.body.custom_prompt.trim()
  ...
  app.post("/api/analyze/stream", analyzeRateLimit, upload.array("images", 20), async (req, res) => {
    const authHeader = req.header("authorization") ?? "";
    const apiKey = authHeader.startsWith("Bearer ")
      ? authHeader.slice(7).trim()
      : "";
    ...
    const alignedToDark = String(req.body.aligned_to_dark ?? "true") !== "false";
    const vcgBand =
      typeof req.body.vcg_band === "string" &&
      (VCG_BAND_OPTIONS as readonly string[]).includes(req.body.vcg_band)
        ? req.body.vcg_band
